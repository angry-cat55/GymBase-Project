...
(function()
{
    return function(path)
    {
        ...
        this.lfn_alert = function(sMsgId,arrWord){
        	var sMsgType = nexacro.getApplication().gdsMessage.lookup("MSG_ID",sMsgId,"MSG_TYPE");
        ...
        this.lfn_getMsg= function(sMsgId, arrWord){
        	...
        	var nRow  = objApp.gdsMessage.findRow("MSG_ID",sMsgId);
        	var sMsg  = objApp.gdsMessage.getColumn(nRow,"MSG_TEXT");
        	var sMsg2 = objApp.gdsMessage.lookup("MSG_ID",sMsgId,"MSG_TEXT");

        	if(!this.lfn_isNull(arrWord)){
        		for(var i=0;i<arrWord.length;i++){
        			var sTemp = "{"+ i.toString() +"}";//{0},{1},{2},{3}
        			sMsg=sMsg.replace(sTemp,arrWord[i]);
        		}
        	}
        	return sMsg;
        }
