...
/***** 엔진 및 런처 정보 *****/
//런처 서비스 버전 정보
var strLauncherVersion = "1,1,0,47";
...
//엔진 버전 정보      
var strVersion = "24.0.0.701";   
      
//넥사크로 N 기본 엔진 key (window32bit/window64bit)
var strEnginesetupkey32 = "{4B8DB8B0-0FB6-4255-90AD-11E5A1692EDB}";
var strEnginesetupkey64 = "{4CFEF184-BAED-42a6-B61E-A93938C9F07E}";
   
//폴더 분리한 엔진 key (window32bit/window64bit)
var usrEnginesetupkey32 = "{D68A5055-4119-412B-B359-C59A920FAFF3}";
var usrEnginesetupkey64 = "{EAB68191-3572-47FA-B35C-7FC96940DF50}";
   
//기본 엔진 Key 정보
var strEnginesetupkey = strEnginesetupkey32;

//폴더 분리용 엔진 key 정보
var usrEnginesetupkey = usrEnginesetupkey32;

//넥사크로 N 기본 엔진 파일명 (window 32bit/window 64bit)
var strEngineFileName32 = "NexacroN_SetupRuntime_x86.exe";
var strEngineFileName64 = "NexacroN_SetupRuntime_x64.exe";

//생성한 엔진 설치 파일명
var usrEngineFileName32 = "EduNexacroN_SetupRuntime_x86.exe";
var usrEngineFileName64 = "EduNexacroN_SetupRuntime_x64.exe";
...
//생성된 엔진 파일명
var usrEngineFileName = usrEnginesetupkey32;
...
if(b64bitUse)
{
	...
	//폴더 분리용 엔진 key 정보
	usrEnginesetupkey = usrEnginesetupkey64;
	
	//기본 엔진 파일명
	strEngineFileName = strEngineFileName64;
	
	//생성된 엔진 파일명
	usrEngineFileName = usrEngineFileName64;
}	
  
/***** 서버 경로 *****/
//웹 서비스 Root 경로
var strServerPath = window.location.protocol + "//" + window.location.host +  "/edupack_egov/";

//런처 서비스 및 엔진 파일 경로
var strNexaFilePath = "./engine";
var bEngineCheck = false;

//이미지 및 아이콘 파일 경로
var strLoadingimage 	= strServerPath + "nexacroN24_ins/img/loading_img.gif";   
var strSplashimage 		= strServerPath + "nexacroN24_ins/img/loading_img.gif";
var strWallpapericon 	= strServerPath + "nexacroN24_ins/img/xicon2.ico";  

 
/***** 프로젝트 속성 정보 *****/  
//project key value   
var strKey = "edupack_nexacro24";
        
//프로젝트 부트스트랩(start.json) 파일 경로
var strBjson = strServerPath + "edupack_nexacro24/_windows_/start.json";
  
//exe 설치 파일이 기본 파일이 아닐때 (SetupResource.zip 파일이 추가로 필요할때 bSetupSource = true, 필요하지 않을때 false)
var bSetupSource = true;
            
//SetupResource.zip 파일 위치 경로
  
var strSetupResourcePath = strServerPath + "nexacroN24_ins/engine";
 
//Component 경로 (%USERAPP% = C:\Documents and Settings\[UserName]\Local Settings\Application Data\TOBESOFT\Nexacro N\24)
var strComponentPath = "%USERAPP%\\Component\\"+strKey+"\\";
...
var jsonDebugMode = {debug:false,loglevel:"debug"};
...
//엔진 파일 설치 여부 알림
function fnDownloadEngine(strType)
{	
	var strMsg = "설치 되지 않았습니다.";
	...
	var strContents = "Engine이 " +strMsg+ "다운로드 후 설치를 진행해 주시기 바랍니다.";
	
	var strSetupEngineName = "";
	
	if(typeof(bSetupSource) !== undefined && bSetupSource === true)
	{
		//엔진 설치 폴더 변경
		strSetupEngineName = usrEngineFileName;			
	}
	else
	{
		//엔진 기본 설치
		strSetupEngineName = strEngineFileName;
	}
	
	var strDownloadPath = strNexaFilePath + "/" + strSetupEngineName;
...
function fnCallBackSendData() 
{
	...
	if(objNexacro.action) 
	{
		...
		else if(strAction == "check")
		{
			...
			//setupResource.zip 파일 다운로드가 필요할 때 (폴더 분리시 변경된 엔진키)
			if(typeof(bSetupSource) !== undefined && bSetupSource === true)
			{
				strEnginesetupkey = usrEnginesetupkey;
			}
		...
		else if(strAction == "setproperty") 
		{
			...
			else if(typeof(bMakeShortcut) == undefined) 
			{
				// 단축아이콘 생성
		...
		else if(strAction == "method") 
		{
			if(objNexacro.result == "success") 
			{
				if(objNexacro.value.downloadresource)
				{	
					bEngineCheck = true;
				...
				//바로가기 아이콘 생성
				else if(objNexacro.value.makeshortcut) 
				{
				...
				{
					//사용할 엔진과 설치된 엔진 버전이 다를경우
					var reg = new RegExp(strVersion, "i");			
					var strRtnEngineVer = objNexacro.value.getengineversion.result;					
					var strEngine = reg.test(strRtnEngineVer);
				
					var strEngineVersion = strVersion.replace(/\./g, ',');
					var strLocalVersion = strRtnEngineVer.replace(/\./g, ',');
								
					var nUpdatecheck = fnCompareVersion(strEngineVersion,strLocalVersion,null);
					...
					else if(nUpdatecheck == 1)
					{
		
					
						fnDownloadEngine("update");				
					}
					else
					{
						...
						if(typeof(bSetupSource) !== undefined && bSetupSource === true)
						{
							...
							fnSetActionProperty('setproperty','division'); 
							fnSendData(true);		
						}
						else
						{
							// nexacro 구동을 위한 기본 속성 설정
							fnSetActionProperty('setproperty','basic'); 
							fnSendData(true);	
						}
...
function fnCompareVersion(v1, v2, options) 
{
...
//Property Action 정보 설정 함수
function fnSetActionProperty(strAction,strType) 
{
