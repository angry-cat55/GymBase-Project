...
//            sColName = rsmd.getColumnName(i).toUpperCase();		
            sColName = rsmd.getColumnLabel(i).toUpperCase();	// alias 사용시	
...
<%
    ...
    try {    
        ...
        try { 
            ...
            String SQL;
            SQL = " SELECT DEPT_CODE, DEPT_NAME FROM TBL_DEPARTMENT";			
            rs = stmt.executeQuery(SQL);
            out_pData.addDataSet(RsToDs(rs,"out_dept"));		
			
            SQL = " SELECT DET_CODE AS CODE , DET_NAME AS NAME FROM TBL_COMMON_CODE WHERE MST_CODE = 'POSITION'";
            rs = stmt.executeQuery(SQL);
            out_pData.addDataSet(RsToDs(rs,"out_pos"));		
        
            SQL = " SELECT DET_CODE AS CODE , DET_NAME AS NAME FROM TBL_COMMON_CODE WHERE MST_CODE = 'HOBBY'";
            rs = stmt.executeQuery(SQL);
            out_pData.addDataSet(RsToDs(rs,"out_hobby"));		

            SQL = " SELECT DET_CODE AS CODE , DET_NAME AS NAME FROM TBL_COMMON_CODE WHERE MST_CODE = 'SKILL'";
            rs = stmt.executeQuery(SQL);
            out_pData.addDataSet(RsToDs(rs,"out_skill"));		
